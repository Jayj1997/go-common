...
// Next moves the iterator to the next element and returns true if there was a next element int the container.
...
func (iterator *Iterator) Next() bool {
	...
	}
	{
		...
		// Above assures that we have reached a leaf node,
		// so return the next entry in current node (if only)
	...
	// Reached leaf node and there are no entries to the right of the current entry
	// so go up to the parent
	for iterator.node.Parent != nil {

		iterator.node = iterator.node.Parent
		// Find next entry position in current node
		// (note: search returns the first equal or bigger than entry)
		e, _ := iterator.tree.search(iterator.node, iterator.entry.Key)
...
// Previous moves the iterator to the previous element and returns true
// if there was a previous element in the container.
// if Previous() returns true, then previous elements's key and value
// can be retrieved by Key() and Value()
// modifies the state of the iterator.
func (iterator *Iterator) Previous() bool {
	...
	}
	{
		...
		// Above assures that we have reached a leaf node,
		// so return the previous entry in current node (if any)
		if e-1 >= 0 {
	...
	// Reached leaf node and there are no entries to the left of the current entry
	// so go up to the parent
	for iterator.node.Parent != nil {

		iterator.node = iterator.node.Parent
		// Find previous entry position in current node
		// (note: search returns the first equal or bigger than entry)
		e, _ := iterator.tree.search(iterator.node, iterator.entry.Key)
...
// First moves the iterator to the first element and
// returns true if there was a first element in the container.
// If First() return true, then first element's key and value can be retrieved by Key() and Value().
// Modifies the state of the iterator
